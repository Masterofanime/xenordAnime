...
function RenderServerBtn() {
   ...
   // 2. Logic: If no valid servers exist, don't show buttons
   if (validSub.length > 0) {
      validSub.forEach(item => subServers.appendChild(createBtn(item, 'sub')));
   } else {
      subServers.innerHTML = '<span class="no-srv">Unavailable</span>';
      showNotification("error", "Please select a server first!");
   }

   if (validDub.length > 0) {
      validDub.forEach(item => dubServers.appendChild(createBtn(item, 'dub')));
   } else {
      dubServers.innerHTML = '<span class="no-srv">Unavailable</span>';
   }
}

function setSelected(serverType, language) {
   // Ensure we format the name correctly for the API (hd-1 or hd-2)
   selectedServer.serverType = serverType.toLowerCase();
   selectedServer.language = language.toLowerCase();

   // Save selected server to localStorage so it remembers the user preference
...
function renderRelatedList() {
   // 1. Get the main parent section (Change this selector to match your HTML section class/ID)
   const relatedSection = document.querySelector('.related-anime-section'); 
   const listContainer = document.getElementById('related-list-container');
   const showMoreBtn = document.getElementById('show-more');
   const counterDisplay = document.getElementById('related-counter');

   const fullData = (typeof animeDetile !== 'undefined' && animeDetile.data && animeDetile.data.related) ?
      animeDetile.data.related :
      [];

   // 2. LOGIC: If total related anime is 0, hide the entire section and stop
   ...
   const displayItems = isRelatedExpanded ? fullData : fullData.slice(0, 4);
   
   if (counterDisplay) {
      counterDisplay.innerText = `Showing ${displayItems.length} of total ${fullData.length} related anime`;
   }
...
if (showMoreBtn) {
  showMoreBtn.style.display = fullData.length <= 4 ? 'none' : 'flex';
